...
for(var i = 0; i < keys.length; i++) {
 keys[i].onclick = function(e) {
  ...
  // Now, just append the key values (btnValue) to the
  // input string and finally use javascript's eval function to get the result
  ...
  // If eval key is pressed, calculate and display the result
   ...
   // This can be done easily using regex and the 'g' tag which will
  ...
  //'decimalAdded' which we'll set once the decimal is added and prevent more decimals to be 
  //added once it's set. It will be reset when an operator, eval or clear key is pressed.
  else if(btnVal == '.') {
   if(!decimalAdded) {
    input.innerHTML += btnVal;
    decimalAdded = true;
